...
from .gpt_cache import get_model_file_path
from .gpt_cache import get_spacy_file_path
...
class OpenAIGPTTokenizer(object):
    ...
    def from_pretrained(cls, config, *inputs, **kwargs):
        tokenizer = cls(config, *inputs, **kwargs)
        return tokenizer

    def __init__(self, config, special_tokens_lt=None, max_len=None):

        self.cache = {}
        self.use_spacy = config["use_spacy"]
        self.nlp_type = ""
        self._load_tokenizer(special_tokens_lt, config)
        self.max_len = _get_max_len(max_len)
        self.encoder = _get_encoder(config)
        self.decoder = _get_decoder(self.encoder)
        self.bpe_ranks = _get_bpe_ranks(config)
    ...
    def _load_tokenizer(self, special_tokens_lt, config):
        if self.use_spacy:
            self._load_spacy_tokenizer(special_tokens_lt, config)
        else:
            self._load_bert_toenizer(special_tokens_lt)
